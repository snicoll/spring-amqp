...
public abstract class AbstractRabbitListenerContainerFactory<C extends AbstractMessageListenerContainer>
	...
	/**
	 * Base identifier to apply to every {@code MessageListenerContainer}
	...
	public C createListenerContainer(RabbitListenerEndpoint endpoint) {
		if (!StringUtils.hasText(this.id)) {
			this.id = generateDefaultFactoryId();
		}

		C instance = createContainerInstance();

		if (endpoint instanceof AbstractRabbitListenerEndpoint) {
			resolveId((AbstractRabbitListenerEndpoint) endpoint);
		}
	...
	protected String generateDefaultFactoryId() { return ListenerContainerIdGenerator.generateContainerFactoryId(SimpleMessageListenerContainer.class);}

	/**
	 * Fully resolve the id of this endpoint. If the endpoint has no id, an
	 * auto-generated suffix is added to this factory id.
	 * @see RabbitListenerEndpoint#getId()
	 */
	protected void resolveId(AbstractRabbitListenerEndpoint endpoint) {
		if (StringUtils.hasText(endpoint.getId())) {
			endpoint.setId(ListenerContainerIdGenerator.generateId(this.id, endpoint.getId()));
		} else {
			endpoint.setId(ListenerContainerIdGenerator.generateId(this.id, counter.getAndIncrement()));
		}
	}
