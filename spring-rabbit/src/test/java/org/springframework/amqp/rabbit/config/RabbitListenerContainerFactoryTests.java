...
public class RabbitListenerContainerFactoryTests {
	...
	@Before
	public void setup() {
		ListenerContainerIdGenerator.resetCounter();
	}
	...
	public void defaultId() {
		SimpleRabbitListenerEndpoint endpoint = createEndpoint();
		this.factory.createListenerContainer(endpoint);
		assertEquals(SimpleMessageListenerContainer.class.getName()+"#0.0", endpoint.getId());

		SimpleRabbitListenerEndpoint endpoint2 = createEndpoint();
		this.factory.createListenerContainer(endpoint2);
		assertEquals(SimpleMessageListenerContainer.class.getName()+"#0.1", endpoint2.getId());
	}

	@Test
	public void defaultIdSeveralFactories() {
		SimpleRabbitListenerEndpoint endpoint = createEndpoint();
		this.factory.createListenerContainer(endpoint);
		assertEquals(SimpleMessageListenerContainer.class.getName()+"#0.0", endpoint.getId());
		...
		SimpleRabbitListenerEndpoint endpoint2 = createEndpoint();
		factory2.createListenerContainer(endpoint2);
		assertEquals(SimpleMessageListenerContainer.class.getName()+"#1.0", endpoint2.getId());
	}

	@Test
	public void explicitEndpointId() {
		SimpleRabbitListenerEndpoint endpoint = createEndpoint();
		endpoint.setId("myEndpoint");

		this.factory.createListenerContainer(endpoint);
		assertEquals(SimpleMessageListenerContainer.class.getName()+"#0$myEndpoint", endpoint.getId());
	}

	@Test
	public void explicitContainerId() {
		this.factory.setId("myFactoryId");
		SimpleRabbitListenerEndpoint endpoint = createEndpoint();

		this.factory.createListenerContainer(endpoint);
		assertEquals("myFactoryId.0", endpoint.getId());
	}

	@Test
	public void explicitContainerAndEndpointId() {
		this.factory.setId("myFactoryId");
		SimpleRabbitListenerEndpoint endpoint = createEndpoint();
		endpoint.setId("myEndpointId");

		this.factory.createListenerContainer(endpoint);
		assertEquals("myFactoryId$myEndpointId", endpoint.getId());
	}
